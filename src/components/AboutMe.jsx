...
export default function AboutMe() {
  ...
  return (
    <section id="aboutme" className="py-24 px-6 max-w-7xl mx-auto">
      <div className="mb-16">
        <p className="text-blue-700 text-xs tracking-[0.25em] uppercase mb-2">{t.about.eyebrow}</p>
      ...
      <div ref={ref} className={`flex flex-col lg:flex-row gap-12 lg:gap-20 items-start transition-all duration-700 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
        {/* Image */}
        <div className="flex-shrink-0 self-center lg:self-start">
          <div className="relative">
            ...
            <div className="w-64 h-80 md:w-72 md:h-96 rounded-lg overflow-hidden bg-slate-800 border border-slate-700">
              <img src="/assets/me2.jpeg" alt="Javier Leonforte"
                className="w-full h-full object-cover hover:scale-105 transition-transform duration-500"
                onError={(e) => { e.currentTarget.style.display = 'none' }} />
            </div>
            <div className="mt-4 flex gap-3 justify-center">
              {t.about.highlights.map(({ value, label }) => (
                <div key={label} className="flex flex-col items-center bg-slate-800 border border-slate-700 hover:border-blue-700/50 rounded-lg px-3 py-2 transition-colors duration-300">
        ...
        <div className="flex-1">
          <h3 className="text-gray-200 text-2xl font-semibold mb-6">
            {t.about.greeting} <span className="text-blue-500">Javier Leonforte</span>
          </h3>
          ...
          <a href="/assets/CV_JavierLeonforte.pdf" download="CV_JavierLeonforte.pdf" className="btn-primary mt-8 w-fit">
